...
import "./LiquidityReward_2_V2.sol";
...
contract LiquidityReward_HANeP_4_V2 is Ownable, ReentrancyGuard, Pausable {
    ...
    LiquidityReward_2_V2 public constant LIQUIDITY_REWARD = LiquidityReward_2_V2(0x31BCaAae793A76f7E723A9EbEecb63c8d0E029f0); // Address of the LiquidityReward contract.
    ...
    // Address of Uniswap V3 Pools with different fees.
    address public constant BASE_POOL_1 = 0xD7294c3eA1426298A50ceA50dF5bCDE96571C88B; // 0.01% fee
    address public constant BASE_POOL_2 = 0x85C31FFA3706d1cce9d525a00f1C7D4A2911754c; // 0.05% fee
    address public constant BASE_POOL_3 = 0x73B14a78a0D396C521f954532d43fd5fFe385216; // 0.3% fee
    address public constant BASE_POOL_4 = 0x37FFd11972128fd624337EbceB167C8C0A5115FF; // 1% fee
    ...
    function addLiquidity(uint256 _usdPrice) external payable nonReentrant {
        ...
        uint256 wbtcTokenAmount = _swapWETHForWBTC(amount, fee); // Swap WETH for WBTC.
    ...
    function getTokenTransfersToUsers(address _tokenAddress) external view returns (address[] memory, uint256[] memory) {
        ...
        // Internal function to check the pool with the highest WBTC balance.
    function _checkPool() internal view returns (uint24) {
    ...
    function _swapWETHForWBTC(uint256 _wethAmount, uint24 _fee) internal returns (uint256) {
    ...
    function transferProvider(address _user, uint256 _liquidity, uint256 _tokenId, uint256 _wbtcAmount, uint256 _hanepAmount, uint256 _totalRewardReleased, uint256 _unclaimedRewards, uint256 _lastClaimedTime, uint256 _lockupPeriod) external onlyOwner nonReentrant {
        TotalLiquidityInfo storage totalInfo = totalLiquidityInfo[_user];

        suppliedWBTC += _wbtcAmount;
        suppliedHANeP += _hanepAmount;
        totalUnderlyingLiquidity += _liquidity;

        totalInfo.totalLiquidity += _liquidity; 
        totalInfo.totalWbtcAmount += _wbtcAmount; 
        totalInfo.totalHanepAmount += _hanepAmount; 
        totalInfo.referrerReward += _hanepAmount * 2 / percent;
        totalInfo.liquidityReward += _hanepAmount; 
        totalInfo.totalRewardReleased += _totalRewardReleased;
        totalInfo.unclaimedRewards += _unclaimedRewards;
    ...
    event SetPercent(uint256 indexed percent);
