...
contract LiquidityReward_2_V2 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant REWARD_PER_SECOND = 6840463029470850000; // Defines the reward per second.
    ...
    uint256 public constant PRICE = 200000000000; // Default price setting in USD (decimal 8).
    ...
    function transferProvider(address _user, uint256 _liquidity, uint256 _tokenId, uint256 _hanAmount, uint256 _wbtcAmount,uint256 _totalReward, uint256 _unclaimedRewards, uint256 _lastClaimedTime, uint256 _lockupPeriod, bool _hasAddedLiquidity) external onlyOwner nonReentrant {
        LiquidityProvider storage provider = providers[_user];
    ...
    function setHasAddedLiquidity(address[] memory _users, bool _hasAddedLiquidity) external onlyOwner nonReentrant {
        for(uint i = 0; i < _users.length; i++) {
            providers[_users[i]].hasAddedLiquidity = _hasAddedLiquidity;
        }
    }
