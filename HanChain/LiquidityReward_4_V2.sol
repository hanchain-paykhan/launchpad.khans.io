...
contract LiquidityReward_4_V2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function addLiquidity() external payable nonReentrant {
        ...
        fee = _checkPool(); // Check the pool with the highest WBTC balance and set the fee accordingly.
        ...
        totalInfo.referrerReward += amount0 * 2 / percent; // Add the referrer reward to the liquidity provider's referrer reward.
    ...
    function transferProvider(address _user, uint256 _liquidity, uint256 _tokenId, uint256 _hanAmount, uint256 _wbtcAmount,uint256 _totalRewardReleased, uint256 _unclaimedRewards, uint256 _lastClaimedTime, uint256 _lockupPeriod) external onlyOwner nonReentrant {
        ...
        totalInfo.totalHanAmount += _hanAmount;
        totalInfo.totalWbtcAmount += _wbtcAmount; 
        totalInfo.referrerReward += _hanAmount * 2 / percent; 
        totalInfo.liquidityReward += _hanAmount; 
